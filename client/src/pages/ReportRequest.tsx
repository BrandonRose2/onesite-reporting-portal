...
export default function ReportRequest({ source = "OneSite" }: { source?: "OneSite" | "Yardi" }) {
  ...
  useEffect(() => {
    ...
    setFormat(saved?.requestedFormat ?? selectedReport.availableFormats[0] ?? "excel");
  ...
  const confirmAndAuthorize = () => {
    if (!selectedReport && scope !== "sync_my_reports") return;
    mutation.mutate({
      source: runnerSource,
      requestType: scope,
  ...
  return <DashboardLayout><PageHeader eyebrow={`Pull Reports / ${source}`} title={`Request a ${source} report`} description="Choose an approved report, set its supported parameters, and route it to all active properties or one selected property." />
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_330px]"><section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_12px_28px_-22px_rgba(15,35,67,.5)] sm:p-7"><div className="space-y-7">
      ...
      <div className="flex flex-wrap items-center gap-3 border-t border-slate-100 pt-5">{selectedReport && scope !== "sync_my_reports" ? <Button type="button" variant="outline" onClick={saveCurrentDefaults} disabled={saveDefaults.isPending} className="border-violet-300 bg-violet-50 text-violet-900 hover:bg-violet-100">{saveDefaults.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}{savedDefaults.data ? "Update report setup" : "Save report setup"}</Button> : null}<Button onClick={validateDraft} disabled={mutation.isPending || (scope === "generate_property" && !propertyId) || (scope !== "sync_my_reports" && !parameterDefinitions.length)} className="bg-[#0b8775] hover:bg-[#087365]">{mutation.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Send className="mr-2 h-4 w-4" />}Review & authorize OneSite run</Button><p className="text-xs text-slate-500">The local source runner can claim this request only after the final portal confirmation and its secure Edge health check.</p></div>
    </div></section>
