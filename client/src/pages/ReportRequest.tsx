...
export default function ReportRequest({ source = "OneSite" }: { source?: "OneSite" | "Yardi" }) {
  ...
  return <DashboardLayout><PageHeader eyebrow={`Pull Reports / ${source}`} title={`Request a ${source} report`} description="Create a traceable request for the authorized reporting runner. Report credentials and browser session data remain outside this portal." />
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_330px]"><section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_12px_28px_-22px_rgba(15,35,67,.5)] sm:p-7"><div className="space-y-7">
      ...
      <fieldset><legend className="text-sm font-semibold text-slate-950">{scope === "generate_property" ? "4" : "3"}. Optional report parameters</legend><p className="mt-1 text-xs leading-5 text-slate-500">Use this field only for report parameters supported by the authorized runner. Leave as an empty object when none are needed.</p><Textarea value={parametersText} onChange={event => setParametersText(event.target.value)} className="mt-3 min-h-28 font-mono text-xs" spellCheck={false} /></fieldset>
