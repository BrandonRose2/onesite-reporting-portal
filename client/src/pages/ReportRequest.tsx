...
export default function ReportRequest({ source = "OneSite" }: { source?: "OneSite" | "Yardi" }) {
  ...
  return <DashboardLayout><PageHeader eyebrow={`Pull Reports / ${source}`} title={`Request a ${source} report`} description="Choose an approved report, set its supported parameters, and route it to all active properties or one selected property." />
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_330px]"><section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_12px_28px_-22px_rgba(15,35,67,.5)] sm:p-7"><div className="space-y-7">
      ...
      {scope !== "sync_my_reports" ? <fieldset><legend className="flex items-center gap-2 text-sm font-semibold text-slate-950">2. Select an approved {source} report <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-[9px] font-bold uppercase tracking-[0.12em] text-[#087365]">Choose</span></legend><div className="mt-3 grid gap-4 sm:grid-cols-2"><div><Label htmlFor="report-search" className="flex items-center justify-between gap-2">Report catalog <span className="text-[11px] font-medium text-[#087365]">{loadingCatalog ? "Syncing…" : `${catalog.length} active reports`}</span></Label><div className="relative mt-1.5"><Search className="pointer-events-none absolute left-3 top-3 h-4 w-4 text-[#087365]" /><Input id="report-search" value={reportSearch} onFocus={() => setPickerOpen(true)} onChange={event => { setReportSearch(event.target.value); setPickerOpen(true); }} onKeyDown={event => { if (event.key === "Escape") setPickerOpen(false); }} placeholder={`Search ${source} reports, areas, or products`} className={`h-11 pl-9 text-sm ${editableFieldClass}`} aria-controls="report-results" aria-expanded={pickerOpen} /><button type="button" onClick={() => setPickerOpen(open => !open)} className={`mt-2 flex h-11 w-full items-center justify-between rounded-lg px-3 text-left text-sm font-medium outline-none ${editableFieldClass}`}><span className="truncate">{selectedReport?.exactReportName ?? (loadingCatalog ? "Loading approved reports…" : `Select a ${source} report`)}</span><ChevronDown className={`h-4 w-4 shrink-0 text-[#087365] transition-transform ${pickerOpen ? "rotate-180" : ""}`} /></button>{pickerOpen ? <div id="report-results" role="listbox" aria-label={`${source} report results`} onWheel={controlPickerWheel} className="absolute z-20 mt-1 max-h-72 w-full snap-y snap-proximity overscroll-contain overflow-y-auto rounded-xl border border-emerald-300 bg-white p-1 shadow-xl shadow-emerald-950/10">{filteredCatalog.length ? filteredCatalog.map(item => <button key={item.id} type="button" role="option" aria-selected={selectedReport?.id === item.id} onClick={() => { setCatalogId(String(item.id)); setPickerOpen(false); setError(null); }} className={`flex w-full snap-start flex-col rounded-lg px-3 py-2.5 text-left transition-colors ${selectedReport?.id === item.id ? "bg-[#dff5ec] text-[#063e36]" : "hover:bg-[#effaf6]"}`}><span className="text-sm font-medium">{item.exactReportName}</span><span className="mt-0.5 text-[11px] text-slate-500">{[item.reportArea, item.reportLevel, item.product].filter(Boolean).join(" · ") || "OneSite report"}</span></button>) : <p className="px-3 py-4 text-sm text-slate-500">No reports match “{reportSearch}”.</p>}</div> : null}</div><p className="mt-2 text-xs text-[#356e65]">{filteredCatalog.length} matching report{filteredCatalog.length === 1 ? "" : "s"}. Type to filter, then choose a report from the green selector.</p></div><div><Label>Allowed formats</Label><div className="mt-1.5 flex h-11 gap-2">{(["excel", "pdf", "csv"] as const).map(item => <button key={item} disabled={!selectedReport?.availableFormats.includes(item)} onClick={() => setFormat(item)} type="button" className={`rounded-lg border px-3 text-xs font-semibold capitalize transition-colors ${format === item ? "border-[#0b8775] bg-[#dff5ec] text-[#087365] shadow-sm" : "border-emerald-200 bg-emerald-50/70 text-slate-500 hover:border-emerald-300"} disabled:cursor-not-allowed disabled:opacity-40`}>{item}</button>)}</div></div></div>
