...
export default function SourceDocumentPreview() {
  ...
  const displaySourceFilename = document.originalFilename.replace(/delinquent\s+and\s+prepaid/gi, "Delinquency");
  return <div className="space-y-6">
    ...
    <Panel eyebrow="Parsed report data" title={`${rows.length} current-resident ledger rows`}><div className="overflow-x-auto"><table className="w-full min-w-[800px] text-left text-xs"><thead className="bg-[#194765] text-[10px] font-bold uppercase tracking-[0.1em] text-white"><tr>{["Unit / resident", "Status", "Transaction", "Delinquent", "Net balance", "90+ days"].map(label => <th key={label} className="px-4 py-3 first:px-5">{label}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">{rows.map(row => <tr key={row.id} className="align-top text-slate-600"><td className="px-5 py-3"><p className="font-semibold text-[#122b4b]">{row.unit ?? "—"}</p><p className="mt-1">{row.residentName ?? "—"}</p></td><td className="px-4 py-3">{row.residentStatus ?? "—"}</td><td className="px-4 py-3"><p>{row.transactionCode ?? "—"}</p><p className="mt-1 text-slate-400">{row.codeDescription ?? ""}</p></td><td className="px-4 py-3 text-right whitespace-nowrap">{currency(row.totalDelinquent)}</td><td className="px-4 py-3 text-right font-semibold whitespace-nowrap text-[#122b4b]">{currency(row.netBalance)}</td><td className="px-4 py-3 text-right whitespace-nowrap text-[#b44851]">{currency(row.days90PlusAmount)}</td></tr>)}</tbody></table></div></Panel>
  </div>;
}
