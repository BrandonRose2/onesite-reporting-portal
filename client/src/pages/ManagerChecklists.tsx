...
import { AlertCircle, Check, CheckCircle2, ChevronRight, ClipboardCheck, Download, ExternalLink, Eye, EyeOff, FileSpreadsheet, Loader2, Mail, Phone, Save, Send, Smartphone, UserRound } from "lucide-react";
...
function isPendingZeroBalance(item: ReviewItem) {
  const reportedValue = item.reportedValue?.trim();
  if (item.status !== "pending" || !reportedValue) return false;
  const normalized = reportedValue.replace(/[$,\s]/g, "").replace(/^\((.+)\)$/, "-$1");
  return /^-?\d+(?:\.\d+)?$/.test(normalized) && Number(normalized) === 0;
}

function requiresActiveManagerReview(item: ReviewItem) {
  return item.requiresVerification !== false && !isPendingZeroBalance(item);
}
...
function blockersFor(state: ReviewState, managerSummary: string): ReviewBlocker[] {
  ...
  if (state.items.some(requiresActiveManagerReview) && !managerSummary.trim()) blockers.push({ itemId: "manager-summary", message: "Add a brief manager summary before submitting for review." });
  return blockers;
}
...
export default function ManagerChecklists() {
  ...
  const [showZeroBalanceRows, setShowZeroBalanceRows] = useState(false);
  ...
  const zeroBalanceRows = useMemo(() => state?.items.filter(isPendingZeroBalance) ?? [], [state]);
  const visibleReviewItems = useMemo(() => state?.items.filter(item => showZeroBalanceRows || !isPendingZeroBalance(item)) ?? [], [state, showZeroBalanceRows]);
  const activeReviewItems = useMemo(() => state?.items.filter(requiresActiveManagerReview) ?? [], [state]);
  const progress = { total: activeReviewItems.length, completed: activeReviewItems.filter(item => item.status !== "pending").length };
