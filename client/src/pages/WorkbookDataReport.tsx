...
function asCurrencyNumber(value: unknown) { const normalized = text(value).replace(/[$,\s]/g, "").replace(/^\((.*)\)$/, "-$1"); const result = Number(normalized); return Number.isFinite(result) ? result : 0; }
...
function selectManagementSheet(sheets: SheetData[]) {
  return sheets.reduce<{ sheet: SheetData; headerIndex: number; score: number } | null>((best, sheet) => {
    const headerIndex = findDataHeader(sheet.rows);
    const nonBlankRows = sheet.rows.filter(row => !isBlankRow(row)).length;
    const score = nonBlankRows + (headerIndex >= 0 ? 100000 : 0);
    return !best || score > best.score ? { sheet, headerIndex, score } : best;
  }, null);
}
...
function UnifiedWorkbookReport({ sheets, reportName }: { sheets: SheetData[]; reportName: string }) {
  const [nonZeroOnly, setNonZeroOnly] = useState(true);
  const selected = useMemo(() => selectManagementSheet(sheets), [sheets]);
  const rows = selected?.sheet.rows ?? [];
  const headerIndex = selected?.headerIndex ?? -1;
  ...
  const balanceIndexes = useMemo(() => delinquencyColumns.filter(field => ["netDelinquency", "current", "days30", "days60", "days90"].includes(field.key)).flatMap(field => { const index = columnIndex(headers, field.candidates); return index === undefined ? [] : [index]; }), [headers]);
  const visibleRows = useMemo(() => !isDelinquency || !nonZeroOnly ? bodyRows : bodyRows.filter(row => row.some(value => normalize(value) === "totals") || balanceIndexes.some(index => asCurrencyNumber(row[index]) !== 0)), [balanceIndexes, bodyRows, isDelinquency, nonZeroOnly]);
  const totals = bodyRows.find(row => row.some(value => normalize(value) === "totals"));
  const metrics = totals && isDelinquency ? delinquencyColumns.filter(field => ["netDelinquency", "current", "days30", "days60", "days90"].includes(field.key)).flatMap(field => { const index = columnIndex(headers, field.candidates); return index === undefined ? [] : [{ label: field.label, value: text(totals[index]) || "—" }]; }) : [];

  return <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_12px_28px_-22px_rgba(15,35,67,.5)]"><div className="border-b border-slate-100 px-4 py-4 sm:px-5"><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#087365]">Unified management report</p><div className="mt-1 flex flex-wrap items-end justify-between gap-3"><div><p className="text-base font-semibold text-slate-900">{isDelinquency ? "Delinquency review" : "Report data review"}</p><p className="mt-0.5 max-w-2xl text-xs leading-5 text-slate-500">{visibleRows.length.toLocaleString()} of {bodyRows.length.toLocaleString()} detail rows shown · {managerFields.length} visible fields. The original workbook retains all {sheets.length} source worksheet{sheets.length === 1 ? "" : "s"}.</p></div>{isDelinquency ? <label className="flex min-h-10 cursor-pointer items-center gap-2 rounded-lg border border-emerald-200 bg-emerald-50 px-3 text-xs font-semibold text-[#075f58]"><Switch checked={nonZeroOnly} onCheckedChange={setNonZeroOnly} aria-label="Show non-zero balances only" /><span>Show non-zero balances only</span></label> : null}</div></div>{metrics.length ? <div className="grid grid-cols-2 border-b border-slate-100 sm:grid-cols-5">{metrics.map(metric => <div key={metric.label} className="border-b border-r border-slate-100 px-3 py-3 last:border-r-0 sm:border-b-0"><p className="text-[9px] font-bold uppercase tracking-[0.08em] text-slate-500">{metric.label}</p><p className="mt-1 truncate text-sm font-semibold text-[#075f58]" title={metric.value}>{metric.value}</p></div>)}</div> : null}<div className="max-h-[68vh] overflow-auto"><table className="min-w-max border-collapse text-left text-xs"><thead><tr><th className="sticky left-0 top-0 z-20 border-b border-r border-slate-200 bg-[#effaf6] px-3 py-2 text-right text-[10px] font-bold text-[#087365]">#</th>{managerFields.map(field => <th key={field.key} className="sticky top-0 z-10 min-w-28 border-b border-r border-slate-200 bg-slate-50 px-3 py-2 align-bottom text-[10px] font-bold uppercase tracking-[0.03em] text-slate-600">{field.label}</th>)}</tr></thead><tbody>{visibleRows.map((row, rowIndex) => <tr key={rowIndex} className="text-slate-600 odd:bg-white even:bg-slate-50/50 hover:bg-emerald-50/60"><th scope="row" className="sticky left-0 z-10 border-b border-r border-slate-200 bg-inherit px-3 py-2 text-right text-[10px] font-bold text-slate-400">{rowIndex + 1}</th>{managerFields.map(field => <td key={field.key} className="max-w-80 border-b border-r border-slate-100 px-3 py-2 align-top whitespace-pre-wrap">{text(row[field.index])}</td>)}</tr>)}</tbody></table>{!visibleRows.length ? <p className="p-8 text-center text-sm text-slate-500">No non-zero delinquency balances were found. Turn off the filter to view all source rows.</p> : null}</div></section>;
}
