...
import { ArrowLeft, ClipboardCheck, Download, FileText, Mail, Printer, ShieldCheck } from "lucide-react";
...
export default function PropertyReportView() {
  ...
  const prepareEmail = () => {
    const subject = `AptCorp Property Report — ${property.name} — ${reportName}`;
    const reportUrl = window.location.href;
    const checklistUrl = `${window.location.origin}/manager-checklists/${property.id}?period=${periodId}`;
    const body = [
      `Hello ${contact?.managerName || "Manager"},`,
      "",
      `The AptCorp Property Report for ${property.name} is ready for your review.`,
      `Report: ${reportUrl}`,
      `Manager checklist: ${checklistUrl}`,
      "",
      "Please review the report, complete the checklist, and save your verification in the portal.",
      "",
      "Thank you,",
      "ApartmentCorp",
    ].join("\n");
    window.location.href = `mailto:${encodeURIComponent(contact?.managerEmail || "")}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
  };
  return <div className="mx-auto max-w-6xl space-y-6 print:max-w-none print:space-y-4">
    <div className="flex flex-wrap items-center justify-between gap-3 print:hidden"><Button variant="outline" onClick={() => setLocation("/history")}><ArrowLeft className="mr-2 h-4 w-4" />Property Reports Library</Button><div className="flex flex-wrap gap-2"><Button variant="outline" onClick={prepareEmail}><Mail className="mr-2 h-4 w-4" />Prepare email</Button><Button variant="outline" onClick={() => window.print()}><Printer className="mr-2 h-4 w-4" />Print / save PDF</Button>{workbook ? <Button className="hunter-metal-button" onClick={openWorkbook} disabled={documentUrlMutation.isPending}><Download className="mr-2 h-4 w-4" />Open workbook</Button> : null}</div></div>
    <article className="overflow-hidden rounded-[1.5rem] border border-[#d9e8e6] bg-white shadow-[0_18px_50px_rgba(16,37,63,0.08)] print:border-0 print:shadow-none"><header className="bg-[#122b4b] p-7 text-white sm:p-9"><p className="text-[11px] font-bold uppercase tracking-[0.16em] text-[#a9d8d1]">AptCorp Property Report</p><div className="mt-3 flex flex-wrap items-start justify-between gap-4"><div><h1 className="text-3xl font-semibold tracking-[-0.04em]">{property.name}</h1><p className="mt-2 text-sm text-slate-200">{reportName} · Property ID {property.externalId} · {property.region}</p></div><Badge className="border border-white/20 bg-white/10 px-3 py-1.5 text-white hover:bg-white/10">{workbook ? `Filed ${filedAt(workbook.createdAt)}` : "Current period review"}</Badge></div></header>
      ...
      <footer className="border-t border-slate-100 bg-slate-50 px-6 py-4 text-xs text-slate-500 sm:px-8"><FileText className="mr-1.5 inline-block h-3.5 w-3.5 text-[#0c7469]" />Generated in AptCorp Property Reports · Use Prepare email to open a manual draft; the portal never sends messages automatically.</footer>
    </article>
  </div>;
}
