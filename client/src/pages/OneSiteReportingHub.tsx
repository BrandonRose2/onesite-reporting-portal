...
export default function OneSiteReportingHub() {
  ...
  const visibleWorkbooks = (documentsQuery.data ?? []).filter(document => !selectedResultsRequestId || document.reportRequestId === selectedResultsRequestId);
  ...
  return <div className="onesite-reporting-hub mx-auto max-w-6xl space-y-6">
    ...
    <section ref={filedWorkbooksRef} id="filed-workbooks" className="scroll-mt-20"><Panel eyebrow="Open workbooks" title={selectedResultsRequestId ? "Workbooks for the selected report" : "All filed property workbooks"}><div className="border-b border-slate-100 p-4 sm:flex sm:items-center sm:justify-between"><p className="text-sm leading-6 text-slate-600">Choose a property and open its workbook. Each link is authorized when you click it.</p>{selectedResultsRequestId ? <Button variant="ghost" size="sm" onClick={() => setSelectedResultsRequestId(null)} className="mt-2 text-[#0c7469] sm:mt-0">Show all workbooks <ArrowRight className="ml-1 h-3.5 w-3.5" /></Button> : null}</div><div className="divide-y divide-slate-100">{visibleWorkbooks.length ? visibleWorkbooks.map(document => <div key={document.id} className="flex flex-wrap items-center justify-between gap-3 p-4"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><Badge variant="outline" className="capitalize">{statusLabel(document.requestStatus)}</Badge><Badge variant="outline">{document.region ?? "Portfolio property"}</Badge></div><p className="mt-2 truncate text-sm font-semibold text-[#122b4b]">{document.propertyName ?? "Portfolio report"}</p><p className="mt-1 truncate text-xs text-slate-500">{document.originalFilename.replace(/delinquent\s+and\s+prepaid/ig, "delinquency")} · {(document.fileSizeBytes / 1024).toFixed(0)} KB · filed {new Date(document.createdAt).toLocaleString()}</p></div><Button type="button" onClick={() => openWorkbook(document.id)} disabled={documentUrlMutation.isPending} className="hunter-metal-button h-9 px-3 text-xs font-semibold text-white">Open workbook</Button></div>) : <div className="p-7 text-center text-sm text-slate-500"><Archive className="mx-auto mb-3 h-6 w-6 text-slate-400" />No filed OneSite workbooks are available for this report yet.</div>}</div></Panel></section>
  </div>;
}
