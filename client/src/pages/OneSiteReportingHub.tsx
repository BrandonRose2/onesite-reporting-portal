...
import { Archive, ArrowRight, CalendarClock, ChevronDown, FileOutput, FolderOpen, History, Mail, Phone, Play, RefreshCw, Settings2, ShieldCheck } from "lucide-react";
...
export const formatOfficePhone = (value: string | null | undefined) => {
  if (!value) return "";
...
export default function OneSiteReportingHub() {
  ...
  const [selectedResultsRequestId, setSelectedResultsRequestId] = useState<number | null>(null);
  const filedWorkbooksRef = useRef<HTMLElement>(null);
  ...
  const showRequestResults = (requestId: number) => {
    setSelectedResultsRequestId(requestId);
    window.setTimeout(() => filedWorkbooksRef.current?.scrollIntoView({ behavior: "smooth", block: "start" }), 0);
  };
  ...
  useEffect(() => {
    ...
    const closeOnOutsideInteraction = (event: PointerEvent) => {
      if (!catalogSelectorRef.current?.contains(event.target as Node)) setCatalogOpen(false);
    };
  ...
  const queueConfiguredReport = () => {
    ...
    if (propertyScope === "specific") {
      const propertyId = Number(contactPropertyId);
      if (!Number.isInteger(propertyId) || propertyId <= 0) {
        toast.error("Choose a specific property before queueing this report.");
        return;
      }
      queueProperty.mutate({ catalogId: selection.id, propertyId, format: selectedFormat, settings });
      return;
    }
    queueCatalog.mutate({ catalogId: selection.id, format: selectedFormat, settings });
  };
  ...
  const activeRequests = (requestsQuery.data ?? []).filter(({ request }) => request.status === "queued" || request.status === "running").slice(0, 2);
  const completedRequests = (requestsQuery.data ?? []).filter(({ request }) => request.status === "completed" || request.status === "completed_with_warnings").slice(0, 4);
  const selectedDocuments = (documentsQuery.data ?? []).filter(document => !selectedResultsRequestId || document.reportRequestId === selectedResultsRequestId);
  const visibleWorkbooks = selectedDocuments.filter(document => document.documentKind === "property_workbook");
  const rawSourceEvidence = selectedDocuments.filter(document => document.documentKind === "source_report");
  const generatedWorkbookCount = (requestId: number) => (documentsQuery.data ?? []).filter(document => document.reportRequestId === requestId && document.documentKind === "property_workbook").length;

  return <div className="onesite-reporting-hub mx-auto max-w-6xl space-y-6">
    <section className="rounded-[1.5rem] bg-[#122b4b] p-6 text-white shadow-[0_18px_50px_rgba(16,37,63,0.18)] sm:p-7"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-[11px] font-bold uppercase tracking-[0.16em] text-[#a9d8d1]">All-property report operations</p><h1 className="mt-2 text-3xl font-semibold tracking-[-0.04em]">Pull Reports – OneSite</h1><p className="mt-2 max-w-3xl text-sm text-slate-200">Choose Delinquency or another approved OneSite report, modify Generate & Schedule settings, and request it for every mapped property.</p></div><div className="flex flex-wrap justify-end gap-2"><Badge className="border border-white/15 bg-white/10 px-3 py-1.5 text-white hover:bg-white/10">All properties</Badge><LiveEdgeReadiness status={liveEdgeStatusQuery.data} isLoading={liveEdgeStatusQuery.isLoading} /></div></div></section>
    ...
    <section className="rounded-[1.25rem] border border-[#cfe4df] bg-white p-5 shadow-[0_10px_24px_rgba(16,37,63,0.06)] sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-[#166249]">Your reports</p><h2 className="mt-1 text-xl font-semibold tracking-[-0.03em] text-[#122b4b]">Run a report, then open its workbooks here</h2><p className="mt-1 max-w-2xl text-sm leading-6 text-slate-600">This is the only place you need to check after pressing Generate. The status changes from waiting, to generating, to ready to open.</p></div><Button variant="outline" onClick={() => { setSelectedResultsRequestId(null); window.setTimeout(() => filedWorkbooksRef.current?.scrollIntoView({ behavior: "smooth", block: "start" }), 0); }} className="border-[#0c7469] text-[#0c7469] hover:bg-[#eaf5f3]"><FolderOpen className="mr-2 h-4 w-4" />All workbooks</Button></div>
      <div className="mt-5 grid gap-4 lg:grid-cols-2"><div className="rounded-xl border border-[#e8ce8d] bg-[#fffaf0] p-4"><p className="text-sm font-semibold text-[#5b4818]">Currently processing</p>{activeRequests.length ? <div className="mt-3 space-y-3">{activeRequests.map(({ request, catalog }) => <div key={request.id} className="rounded-lg border border-[#f0dfb8] bg-white/80 p-3"><div className="flex flex-wrap items-center gap-2"><Badge className="bg-[#8a6d28] text-white hover:bg-[#8a6d28]">{requestStageLabel(request)}</Badge><span className="text-xs font-semibold text-[#122b4b]">{portalReportTitle(catalog?.displayName ?? request.requestedReportName)}</span></div><p className="mt-2 text-xs leading-5 text-slate-600">{requestStageHelper(request)}</p></div>)}</div> : <p className="mt-2 text-xs leading-5 text-slate-600">No reports are processing. Start a new request below when you are ready.</p>}</div><div className="rounded-xl border border-[#b9d8cf] bg-[#f4fbf8] p-4"><p className="text-sm font-semibold text-[#155d55]">Ready to open</p>{completedRequests.length ? <div className="mt-3 space-y-3">{completedRequests.map(({ request, catalog }) => { const workbookCount = generatedWorkbookCount(request.id); return <div key={request.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-[#cfe4df] bg-white/80 p-3"><div><p className="text-sm font-semibold text-[#122b4b]">{portalReportTitle(catalog?.displayName ?? request.requestedReportName)}</p><p className="mt-1 text-xs text-slate-600">{workbookCount ? `${workbookCount} AptCorp-generated ${workbookCount === 1 ? "workbook" : "workbooks"}` : "Preparing AptCorp workbooks"}</p></div><Button size="sm" onClick={() => showRequestResults(request.id)} className="hunter-metal-button"><FolderOpen className="mr-2 h-3.5 w-3.5" />Open workbooks</Button></div>; })}</div> : <p className="mt-2 text-xs leading-5 text-slate-600">Completed reports will appear here with an Open workbooks button.</p>}</div></div>
    </section>
    <section className="grid gap-6 lg:grid-cols-[1.08fr_.92fr]">
      <Panel eyebrow="Request a OneSite report" title="Configure Generate & Schedule"><div className="space-y-5 p-5 sm:p-6">
        ...
        <section className="space-y-4"><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-[#166249]">Step 2 · Set Parameters</p><div className="grid gap-4 rounded-xl border border-slate-200 p-4 sm:grid-cols-2"><div className="space-y-2"><Label>Generation timing</Label><Select value={generationMode} onValueChange={value => setGenerationMode(value as "generate_now" | "schedule_later")}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="generate_now">Generate now</SelectItem><SelectItem value="schedule_later">Schedule for later</SelectItem></SelectContent></Select></div><div className="space-y-2"><Label>Requested export format</Label><Select value={selectedFormat} onValueChange={value => setSelectedFormat(value as "excel" | "pdf" | "csv")}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="excel">Excel</SelectItem><SelectItem value="pdf">PDF</SelectItem><SelectItem value="csv">CSV</SelectItem></SelectContent></Select></div>{generationMode === "schedule_later" ? <div className="space-y-2 sm:col-span-2"><Label htmlFor="scheduled-at">Date and time</Label><Input id="scheduled-at" type="datetime-local" value={scheduledForLocal} onChange={event => setScheduledForLocal(event.target.value)} /></div> : null}</div><StructuredReportSettings fields={parseSettingsSchema(selection?.settingsSchemaJson)} values={reportParameters} onChange={setReportParameters} /></section>
        <section className="space-y-3"><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-[#166249]">Step 3 · Choose Property</p><div className="grid gap-3 sm:grid-cols-2"><button type="button" onClick={() => setPropertyScope("all")} className={`rounded-xl border p-4 text-left transition-colors ${propertyScope === "all" ? "border-[#0c7469] bg-[#edf9f4] shadow-sm" : "border-slate-200 bg-white hover:bg-slate-50"}`}><p className="font-semibold text-[#122b4b]">All properties</p><p className="mt-1 text-xs leading-5 text-slate-600">Generate one report output for every mapped OneSite property.</p></button><button type="button" onClick={() => setPropertyScope("specific")} className={`rounded-xl border p-4 text-left transition-colors ${propertyScope === "specific" ? "border-[#0c7469] bg-[#edf9f4] shadow-sm" : "border-slate-200 bg-white hover:bg-slate-50"}`}><p className="font-semibold text-[#122b4b]">Specific property</p><p className="mt-1 text-xs leading-5 text-slate-600">Generate and file the selected report for one mapped property.</p></button></div>{propertyScope === "specific" ? <PropertyContactAutofill contacts={propertyContactsQuery.data ?? []} selectedId={contactPropertyId} onSelect={setContactPropertyId} /> : <p className="rounded-xl border border-dashed border-[#b8d7d0] bg-white/70 p-3 text-xs leading-5 text-slate-600">All mapped OneSite properties will be included. Switch to Specific property to choose one property and its contact details.</p>}</section>
        <section className="rounded-xl border border-[#9acdbf] bg-[#edf9f4] p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-[#166249]">Step 4 · Generate</p><h3 className="mt-1 text-base font-semibold text-[#122b4b]">{propertyScope === "specific" ? `Generate for ${selectedPropertyContact?.propertyName ?? "selected property"}` : "Generate for all mapped properties"}</h3><p className="mt-1 text-xs leading-5 text-slate-600">{propertyScope === "specific" ? "This queues the selected report for the one chosen property." : "This queues the selected report across every mapped OneSite property."}</p></div><Badge variant="outline" className="border-[#6eb8a5] bg-white text-[#166249]">{propertyScope === "specific" ? "Specific property" : "All properties"}</Badge></div><Button disabled={!selection || !liveEdgeReady || queueCatalog.isPending || queueProperty.isPending || (propertyScope === "specific" && !contactPropertyId)} onClick={queueConfiguredReport} className="hunter-metal-button mt-4 w-full"><CalendarClock className="mr-2 h-4 w-4" />{queueCatalog.isPending || queueProperty.isPending ? "Saving settings and queueing…" : !liveEdgeReady ? "Open signed-in Edge to queue" : generationMode === "schedule_later" ? `Schedule report for ${propertyScope === "specific" ? "selected property" : "all properties"}` : `Generate report for ${propertyScope === "specific" ? "selected property" : "all properties"}`}</Button></section>
        <details className="rounded-xl border border-slate-200 bg-slate-50 p-4"><summary className="cursor-pointer text-sm font-semibold text-[#122b4b]">Advanced request options</summary><div className="mt-4 space-y-4"><div className="space-y-2"><Label htmlFor="external-emails">External completion emails</Label><Input id="external-emails" value={externalEmails} onChange={event => setExternalEmails(event.target.value)} placeholder="manager@example.com; accounting@example.com" /><p className="text-xs text-slate-500">Selecting a property automatically fills its authorized Company Contacts email when available. You may add additional recipients with commas, semicolons, or new lines.</p></div>
        ...
        <p className="text-xs leading-5 text-slate-500">The request records the selected title, property scope, timing, output format, delivery choices, and parameter settings before the trusted OneSite runner begins work.</p>
        ...
        {showCustom ? <div className="grid gap-3 rounded-xl border border-slate-200 p-4 sm:grid-cols-[1fr_130px]"><div className="space-y-2"><Label htmlFor="custom-title">Exact OneSite report title</Label><Input id="custom-title" value={customTitle} onChange={event => setCustomTitle(event.target.value)} placeholder="Use the title displayed in OneSite Reports" /></div><div className="space-y-2"><Label>Format</Label><Select value={customFormat} onValueChange={value => setCustomFormat(value as "excel" | "pdf" | "csv")}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="excel">Excel</SelectItem><SelectItem value="pdf">PDF</SelectItem><SelectItem value="csv">CSV</SelectItem></SelectContent></Select></div><Button className="hunter-metal-button sm:col-span-2" disabled={!customTitle.trim() || !liveEdgeReady || queueCustom.isPending} onClick={() => queueCustom.mutate({ exactReportName: customTitle, format: customFormat })}>Queue custom all-property request</Button></div> : null}</div></details>
      </div></Panel>
      <Panel eyebrow="Manual OneSite work" title="Already generated a report in OneSite?"><div className="space-y-4 p-5 sm:p-6"><p className="text-sm leading-6 text-slate-600">Only use this if you ran a report directly in OneSite rather than through this portal. The normal path is: Run report above, then open its workbooks from Your reports.</p><Button onClick={() => syncMyReports.mutate()} disabled={!liveEdgeReady || syncMyReports.isPending} variant="outline" className="w-full border-[#0c7469] text-[#0c7469] hover:bg-[#eaf5f3]"><RefreshCw className={`mr-2 h-4 w-4 ${syncMyReports.isPending ? "animate-spin" : ""}`} />{syncMyReports.isPending ? "Queueing synchronization…" : !liveEdgeReady ? "Open signed-in Edge to sync" : "Find manually generated reports"}</Button></div></Panel>
    </section>
    <section ref={filedWorkbooksRef} id="filed-workbooks" className="scroll-mt-20"><Panel eyebrow="Open workbooks" title={selectedResultsRequestId ? "AptCorp workbooks for the selected report" : "All AptCorp property workbooks"}><div className="border-b border-slate-100 p-4 sm:flex sm:items-center sm:justify-between"><p className="text-sm leading-6 text-slate-600">Open the AptCorp-created workbook for a property. Raw OneSite XLS files remain available only as source evidence.</p>{selectedResultsRequestId ? <Button variant="ghost" size="sm" onClick={() => setSelectedResultsRequestId(null)} className="mt-2 text-[#0c7469] sm:mt-0">Show all workbooks <ArrowRight className="ml-1 h-3.5 w-3.5" /></Button> : null}</div><div className="divide-y divide-slate-100">{visibleWorkbooks.length ? visibleWorkbooks.map(document => <div key={document.id} className="flex flex-wrap items-center justify-between gap-3 p-4"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><Badge className="bg-[#0c7469] text-white hover:bg-[#0c7469]">AptCorp workbook</Badge><Badge variant="outline">{document.region ?? "Portfolio property"}</Badge></div><p className="mt-2 truncate text-sm font-semibold text-[#122b4b]">{document.propertyName ?? "Portfolio report"}</p><p className="mt-1 truncate text-xs text-slate-500">Generated from the filed OneSite source · {(document.fileSizeBytes / 1024).toFixed(0)} KB · filed {new Date(document.createdAt).toLocaleString()}</p></div><Button type="button" onClick={() => openWorkbook(document.id)} disabled={documentUrlMutation.isPending} className="hunter-metal-button h-9 px-3 text-xs font-semibold text-white">Open workbook</Button></div>) : <div className="p-7 text-center text-sm text-slate-500"><Archive className="mx-auto mb-3 h-6 w-6 text-slate-400" />AptCorp workbooks are being prepared for this report.</div>}</div>{rawSourceEvidence.length ? <details className="border-t border-slate-100 p-4"><summary className="cursor-pointer text-sm font-semibold text-slate-600">View retained raw OneSite source evidence ({rawSourceEvidence.length})</summary><div className="mt-3 space-y-2">{rawSourceEvidence.map(document => <div key={document.id} className="flex items-center justify-between gap-3 rounded-lg border border-slate-200 p-3 text-xs"><span className="min-w-0 truncate text-slate-600">{document.propertyName ?? "Portfolio report"} · {document.originalFilename}</span><Button type="button" size="sm" variant="outline" onClick={() => openWorkbook(document.id)} disabled={documentUrlMutation.isPending}>Open source</Button></div>)}</div></details> : null}</Panel></section>
  </div>;
}
