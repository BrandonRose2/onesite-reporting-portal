  ...
  return <div className="onesite-reporting-hub mx-auto max-w-6xl space-y-6">
    ...
    <section className="grid gap-6 lg:grid-cols-[1.08fr_.92fr]">
      <Panel eyebrow="Request a OneSite report" title="Configure Generate & Schedule"><div className="space-y-5 p-5 sm:p-6">
        ...
        <section className="space-y-3"><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-[#166249]">Step 3 · Choose Property</p><PropertyContactAutofill contacts={propertyContactsQuery.data ?? []} selectedId={contactPropertyId} onSelect={setContactPropertyId} /><p className="text-xs leading-5 text-slate-500">The selected property sets contact and delivery details. The supported OneSite generation scope for this workflow is shown in the final step.</p></section>
        <section className="rounded-xl border border-[#9acdbf] bg-[#edf9f4] p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-[#166249]">Step 4 · Generate</p><h3 className="mt-1 text-base font-semibold text-[#122b4b]">Generate for all mapped properties</h3><p className="mt-1 text-xs leading-5 text-slate-600">This queues the selected report across every mapped OneSite property. The property selected above controls optional contact and delivery details only.</p></div><Badge variant="outline" className="border-[#6eb8a5] bg-white text-[#166249]">All properties</Badge></div><Button disabled={!selection || !liveEdgeReady || queueCatalog.isPending} onClick={queueConfiguredReport} className="hunter-metal-button mt-4 w-full"><CalendarClock className="mr-2 h-4 w-4" />{queueCatalog.isPending ? "Saving settings and queueing…" : !liveEdgeReady ? "Open signed-in Edge to queue" : generationMode === "schedule_later" ? "Schedule report for all properties" : "Generate report for all properties"}</Button></section>
