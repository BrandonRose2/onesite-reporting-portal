...
export function ReportSummaryViewer({ html, reportName }: { html: string; reportName: string }) {
  const [open, setOpen] = useState(false);
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button variant="outline" size="sm" className="mt-2 w-full sm:w-auto">Open detailed HTML summary</Button></DialogTrigger><DialogContent className="flex h-[100dvh] w-screen max-w-none flex-col overflow-hidden rounded-none p-0 sm:h-[86vh] sm:max-w-5xl sm:rounded-lg"><DialogHeader className="border-b border-slate-200 px-4 py-4 sm:px-6 sm:py-5"><DialogTitle className="text-sm sm:text-base">{reportName} — detailed summary</DialogTitle><DialogDescription className="text-xs">Generated from the runner completion record. Original report files remain separately preserved.</DialogDescription></DialogHeader><iframe title={`${reportName} detailed summary`} sandbox="" srcDoc={html} className="min-h-0 flex-1 border-0 bg-white" /></DialogContent></Dialog>;
}
